...
**The working tree is no longer that package, and as of lap 4 it is no longer transitional either.** All
three projects target `net10.0` alone and reference `ProphetsWay.BaseDataAccess` 3.1.0, no project
references EF6, `BaseEFDataAccess` implements the 3.x disposal contract, and **the library is now exactly
the twelve-class 3.0.0 surface plus one enum and two internal helpers — nothing else.** `CHANGELOG.md`
opens with a **v3.0.0** entry describing it.
...
### 3.x Progress — all four laps have landed
...
`d00aad3` is **HEAD** — read from `.git/logs/HEAD` on 2026-08-22, where its subject is *"Delete the 2.2.x
DAO surface superseded by the open-key families."* Its file-by-file stat was supplied by the owner and
could not be re-run here (no terminal); **its effect was verified directly against the tree** and that is
the stronger check — see the two paragraphs below.
...
**What has *not* landed, now that lap 4 has:**

- **Provider neutrality (FR 7 / D2) — the packaging half only.** `Microsoft.EntityFrameworkCore.SqlServer`
  and `.InMemory` are still `PackageReference`s in `ProphetsWay.EFTools.csproj`, re-verified by opening it
  2026-08-22. Lap 4 did not touch that file. **The *code* half is done** — see deviation 3.
- **The SQLite / SQL Server certification legs (FR 11).** SQLite in-memory is already in real use by the
  locally written tests; what is missing is a *certified* two-provider run of the whole suite. See
  deviation 4, which used to state this wrongly.
- **A green suite.** 11 failures remain, all pre-existing. See deviation 4.
- **The tag and the publish.** 3.0.0 is set in `app-variables.yml` and not yet released.
...
**Build and test state — reported by the owner on 2026-08-22 *after* lap 4, not re-measured here.**
`dotnet build` **0 errors / 0 warnings**; lap gate `--filter "Area=Keyless|Area=Insert"` **68 / 68**; full
`ProphetsWay.EFTools.Tests` **270 total / 259 passed / 11 failed**; `ProphetsWay.Example.Tests`
**164 / 164** on both `net10.0` and `net48`. **The 11 are the same pre-existing set, by name** — ten in
`EFSnapshotDeepCopyTests` (that upstream class in its entirety) plus
`EFDataAccessTransactionTests.ShouldExposeUncommittedWritesToAnotherInstance`. **Lap 4 changed no counts
and fixed and broke nothing**, which is what a pure deletion of unreferenced files should do and is the
reason these figures survive it unchanged. The 68 and the 270 are **independently corroborated by static
count** — see deviation 4 — which is the strongest check available without a terminal. The earlier
figures of *7 warnings* and *151 / 123 / 28* are superseded; do not restate either.
...
| `ProphetsWay.EFTools.Tests/` | **25 source files** — 13 one-line adapters, **8 test classes written directly against this library's own surface**, 2 seam guards, `TestSeam.cs` and `Constants.cs`. **The figure of 7 that this row and deviation 4 both carried was wrong** — the eighth is `CompanyResourceConversionTests`, which the same sentence went on to list. See deviation 4 |
...
| `RootNonIdDao<TEntity>` | public abstract class | Keyless plumbing that **implements no capability interface**, so deriving from it commits to nothing. `MatchRow` is its one abstract member and **the only abstract member in the library** — re-verified after lap 4 by grepping every `protected`/`public` member declaration in all 15 files; `BaseDao.MatchRow` is `protected virtual`, `RootNonIdDao.MatchRow` is `protected abstract`, and nothing else is abstract. `ApplyStableOrder` throws until overridden. **`GetCore` and `UpdateCore` are declared here, not on `BaseDao`** — `RootNonIdDao.cs` lines 308 and 342, overridden in `RootSoftNonIdDao.cs` at 201 and 229. `BaseDao`'s own protected seams are `TrackForWrite`, `ApplyUpdateValues`, `GetKey`, `MatchRow`, `KeyEquals`, `ApplyReadFilter`, `ApplyIncludes` and `ApplyStableOrder` — and **no `GetCore` or `UpdateCore`** |
