...
**The working tree is no longer that package.** All three projects target `net10.0` alone and reference
`ProphetsWay.BaseDataAccess` 3.1.0, no project references EF6, `BaseEFDataAccess` implements the 3.x
disposal contract, and **the twelve-class 3.0.0 public surface now exists in the tree**. `CHANGELOG.md`
opens with an unreleased **v3.0.0** entry describing it. `app-variables.yml` still reads `2` / `2` / `0` —
**never change it; the bump is the owner's** — so the published package and the tree describe different
things, and **that mismatch is a known open owner decision, not something for an agent to resolve or hide.**

### 3.x Progress — laps 1–3 have landed; lap 4 has not
...
**Landed in laps 1–3 — verified 2026-08-22 by listing `ProphetsWay.EFTools/` and grepping every type
...
**All twelve classes of the target surface are present.** `docs/api-contract.md` (revision 11) § *The
Public Surface* names them — `BaseEFContext`, `BaseEFDataAccess<TContext>`, the three keyed families, the
three soft keyed families, and the four keyless types — plus the `ContextOwnership` enum. Every one is in
the tree. **The sentence "none of the twelve-class redesign has landed" is dead and must not be
restated.**

**What lap 4 still owes, and what else has not landed:**

- **The 2.2.x shape is still present alongside the new one** — the 18 key-specific closures in
  `ProphetsWay.EFTools.Guid` / `.Int` / `.Long`, both `RootBase*` bridges, the internal `RootDao`, and
  the three `Legacy*` types. That is **24 of the library's 39 source files.** `CHANGELOG.md` says they
  are scheduled for deletion before 3.0.0 is tagged and are not a supported migration target.
- **Provider neutrality (FR 7 / D2).** The `Microsoft.EntityFrameworkCore.SqlServer` and
  `.InMemory` package references are still in `ProphetsWay.EFTools.csproj`. **The *code* half is
  already done** — see deviation 3.
- **The dead `#if NET461 || NET471 || NET48` blocks** survive in exactly those same 24 legacy files, and
  in no lap-1–3 file. They go with the files.
- **The SQLite / SQL Server certification legs (FR 11).** The suite runs on SQL Server alone.
- **A green suite.** See deviation 4.

**Build and test state — reported by the owner on 2026-08-22, not re-measured here.** `dotnet build`
**0 errors / 0 warnings**; lap gate `--filter "Area=Keyless|Area=Insert"` **68 / 68**; full
`ProphetsWay.EFTools.Tests` **270 total / 259 passed / 11 failed**; `ProphetsWay.Example.Tests`
**164 / 164** on both `net10.0` and `net48`. The 68 and the 270 are **independently corroborated by
static count** — see deviation 4 — which is the strongest check available without a terminal. The
earlier figures of *7 warnings* and *151 / 123 / 28* are superseded; do not restate either.
...
Counts re-taken 2026-08-22 by listing each directory. **Every figure in this table changed since the
last pass; the previous 27 / 7 / 19 is superseded.**

| Project | Role |
| --- | --- |
| `ProphetsWay.EFTools/` | Published library; **39 source files** — 21 in the root, 6 each in `Guid/`, `Int/`, `Long/`. One top-level type per file: **34 public abstract classes, 1 public enum, 4 internal classes** (`EntityGraph`, `SoftTimestamps`, `RootDao`, `LegacyRootNonIdDao`). **15 of the 39 are the 3.x surface plus its two internal helpers; the other 24 are the 2.2.x shape lap 4 deletes** |
...
| `ProphetsWay.EFTools.Tests/` | **25 source files** — 13 one-line adapters, **7 test classes written directly against this library's own surface**, 2 seam guards, `TestSeam.cs` and `Constants.cs`. See deviation 4 |
| `ProphetsWay.Example/` | Git submodule tracking the standalone Example repo; never edit it here |

The submodule pointer is at **`61d9e7d`** — `61d9e7dfb209c4a92b0c16d058aad1af08031fb5`, re-read from
`.git/modules/ProphetsWay.Example/HEAD` on 2026-08-22 and **unchanged since 2026-08-18**. **It is not the
`3.1.0` tag** — `git submodule status` describes it as `3.1.0-1-g61d9e7d`, i.e. one commit past that tag:
the merge of `ProphetsWay.Example` PR #21 (`3.1.1-eftool-findings`) on 2026-08-18. That commit opened the
**3.1.1** line and brought in `TestDataAccessFactory.Use`, the seam this repository's harness depends on.
**Earlier text saying `d845863` — "the 3.1.0 tree" — is superseded and must not be restated**; so is any
...
**Two shapes coexist.** The 3.x surface is complete; the 2.2.x surface has not been removed. Everything
below was read from the file named, on 2026-08-22.

#### The 3.x surface — all twelve classes plus the enum are in the tree
...
| `BaseEFDataAccess<TContext>` | public abstract class | The DAL root. **One type parameter, not two** — `BaseEFDataAccess<TContextType,TIdType>` no longer exists, and `README.md` still says it does. Takes a built context plus a `ContextOwnership` through one `protected` constructor; both public constructors are gone. `Dispose` is **`sealed override`**, idempotent, non-throwing, rolls back an open transaction and disposes the context only when `Owned`; `protected virtual DisposeCore()` is the derived hook. **`ObjectDisposedException` guarding is complete** — all **10** non-`Dispose` members (`TransactionStart`/`Commit`/`RollBack`, `GetAll`, `GetPaged`, `GetCount`, `Get`, `Insert`, `Update`, `Delete`) open with `ThrowIfDisposed()`, and `protected ThrowIfDisposed()` is exposed so a derived DAL guards its own forwarders. **The claim that guarding "is not yet in place" is dead** |
...
| `RootNonIdDao<TEntity>` | public abstract class | Keyless plumbing that **implements no capability interface**, so deriving from it commits to nothing. `MatchRow` is its one abstract member and **the only abstract member in the library**; `ApplyStableOrder` throws until overridden |
...
#### The 2.2.x surface — still present, scheduled for deletion in lap 4

| Type | Kind | Role |
| --- | --- | --- |
| `Guid.*`, `Int.*`, `Long.*` DAO bases | 18 public abstract classes | The key-specific closures. Superseded by the open-key families |
| `RootBaseDao<T,TIdType>` / `RootBaseSoftDao<T,TIdType>` | public abstract bridges | `where TIdType : struct`, which is the constraint the open-key families exist to drop |
| `RootDao<T,TIdType>`, `LegacyRootNonIdDao<T>` | internal classes | The 2.2.x engines |
| `LegacyBaseNonIdDao<T>` / `LegacyBaseSoftNonIdDao<T>` | public abstract classes | The 2.2.x keyless bases, **renamed in lap 3 purely to free `BaseNonIdDao` / `BaseSoftNonIdDao` for the new types**. `CHANGELOG.md` says they are not a supported migration target. **Never write new code against anything in this table** |

All 24 of these files — and no lap-1–3 file — still carry `using System.Data.Entity;` inside a dead
`#if NET461 || NET471 || NET48` block. The `#if` blocks and the files are one removal, not two.

**Read [docs/api-contract.md](docs/api-contract.md) as a description of the *target*, and the table above
as a description of the *tree*.** They now agree on the twelve-class surface and disagree on everything
lap 4 deletes — the contract document's *"Types that disappear"* list is still entirely present.
...
| 3 | Runtime package forces SQL Server and InMemory providers — **half closed 2026-08-22: the code half is done, the packaging half is not** | **Medium, and now smaller than this row used to claim.** The **code** is provider-neutral: grepping every `.cs` in `ProphetsWay.EFTools/` for `UseSqlServer` / `UseInMemoryDatabase` returns **nothing**. `BaseEFContext` has one `protected BaseEFContext(DbContextOptions)` constructor and names no provider; `BaseEFDataAccess<TContext>` takes a built context. **The sentence "`UseSqlServer` lives in the published library" is dead** — the provider is now named in the consumer's own file (`ProphetsWay.Example.DataAccess.EF/ExampleDataAccess.cs`). What survives is the **packaging** half: `ProphetsWay.EFTools.csproj` still carries `Microsoft.EntityFrameworkCore.SqlServer` **10.0.11** and `Microsoft.EntityFrameworkCore.InMemory` **10.0.11** alongside `Microsoft.EntityFrameworkCore`, so a consumer on PostgreSQL still transitively restores two providers they do not want. Removing a transitive package reference is breaking, so it lands in 3.0.0 — [FR 7](docs/feature-requests.md), **D2**. |
| 4 | ~~**There is no test suite in this repository at all**~~ · ~~**19 files, 151 cases, nothing tests this library's own surface**~~ **BOTH SUPERSEDED 2026-08-22 — the suite is now 25 files and 270 cases, and 126 of them target this library directly** | **Medium, and narrowing.** **Do not restate either struck heading, and do not restate 151 / 123 / 28.** Re-counted 2026-08-22 by listing `ProphetsWay.EFTools.Tests/` and grepping every `[Fact]`, `[Theory]`, `[InlineData]` and `[Trait]` in it and in `ProphetsWay.Example.Tests/`. **25 source files:** `TestSeam.cs`, an `internal static` class whose `[ModuleInitializer]` calls `TestDataAccessFactory.Use(() => Constants.GetExampleDataAccess)` and so points the **whole upstream `ProphetsWay.Example` suite** at the EF DAL for the assembly's run; **13 adapters**, each literally `public class EFXxxTests : XxxTests { }`; **2 seam guards** — `TestSeamTests.cs` (5 `[Fact]`) and `AdapterCoverageTests.cs` (2 `[Fact]`), both `[Trait("Guard","Seam")]`; **7 test classes written directly against this library's own public surface** — `KeylessDaoTests` (31 cases), `KeylessSoftDaoTests` (29), `KeyPredicateOpenKeyTests` (21), `AlternateKeyGuardSpikeTests` (14), `SoftDeleteTimestampHookTests` (12), `FailedInsertWriteBackTests` (5), `IdentifierResolutionTests` (4), `CompanyResourceConversionTests` (3), each declaring its own entities, `DbContext` and DAOs on `BaseDao` / `BaseSoftDao` / `RootNonIdDao` / `RootSoftNonIdDao` / `BaseEFDataAccess`; and `Constants.cs`, one SQL Server connection string with `TrustServerCertificate=True` and **no `#if` branches**. **Discoverable cases: 270** — **144** from the 13 adapted upstream classes (the 20 `ConventionShowcase` cases are correctly *not* adapted, leaving 164 − 20) plus **126** declared here. Both halves were counted independently and they reconcile with the runner exactly, which is why this row states them without a terminal. **Trait split of the local 126:** `Scope=Contract` 99, `Characterization` 19, `Dispatcher` 1, `Guard=Seam` 7 — **every local case carries exactly one `Scope` or the `Guard` key.** By `Area`: `Keyless` 64, `KeyPredicate` 25, `AlternateKeys` 14, `SoftDelete` 12, `Insert` 4; `Keyless` + `Insert` = **68**, which is the lap gate the owner reported passing 68/68. **Last measured result: 2026-08-22, reported by the owner — 270 total, 259 passed, 11 failed** (10 × `EFSnapshotDeepCopyTests`, which is that upstream class in its entirety, plus one SQL Server transaction-visibility timeout). Not re-run here. The `.trx` files in `TestResults/` are **stale laps from 2026-08-18** — 147/53/94, 147/15/132 and 151/57/94 — and must not be read as current. **What is now covered that this row said was not:** the open-key, soft-delete and keyless families are all exercised directly, and `CompanyResourceDao` **exists** — `ProphetsWay.Example.DataAccess.EF/Daos/CompanyResourceDao.cs` is a real `RootNonIdDao<CompanyResource>`, `ExampleContext` declares `DbSet<CompanyResource>` with `HasKey(x => new { x.CompanyId, x.ResourceId })` and `ToTable("CompanyResources")`, and the three `ICompanyResourceDao` forwarders call it behind `ThrowIfDisposed()`. **The claim that it "does not exist" and dominates the failures is dead.** **What is still not covered:** the 24 legacy files lap 4 deletes are exercised by nothing, which is harmless because they are being removed; provider portability is untested — the suite needs a **local SQL Server** carrying `ProphetsWay.Example` and runs on that provider alone, despite the `Microsoft.EntityFrameworkCore.Sqlite` reference in the test `.csproj` ([FR 11](docs/feature-requests.md)); and `LocalTestsOnly: 'yes'` skips the whole suite in CI. |
